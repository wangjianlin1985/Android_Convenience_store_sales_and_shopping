...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  holder.tv_collectionTime = (TextView) convertView.findViewById(R.id.tv_collectionTime);
	  /*设置各个控件的展示内容*/
	  holder.tv_productObj.setText("收藏宝贝：" + (new ProductService()).GetProduct(Integer.parseInt(mData.get(position).get("productObj").toString())).getProductName());
	  holder.tv_userObj.setText("收藏用户：" + (new UserInfoService()).GetUserInfo(mData.get(position).get("userObj").toString()).getName());
	  holder.tv_collectionTime.setText("收藏时间：" + mData.get(position).get("collectionTime").toString());
