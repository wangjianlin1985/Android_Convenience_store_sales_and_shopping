...
public class SendWayAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.sendway_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加送货方式");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_sendWayName.getText().toString().equals("")) {
						Toast.makeText(SendWayAddActivity.this, "送货方式名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传送货方式信息*/
					SendWayAddActivity.this.setTitle("正在上传送货方式信息，稍等...");
					String result = sendWayService.AddSendWay(sendWay);
