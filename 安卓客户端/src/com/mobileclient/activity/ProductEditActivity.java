...
public class ProductEditActivity extends Activity {
		...
		btnUpdate.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					...
					if (!product.getMainPhoto().startsWith("upload/")) {
					...
					if(ET_price.getText().toString().equals("")) {
						Toast.makeText(ProductEditActivity.this, "商品价格输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_productDesc.getText().toString().equals("")) {
						Toast.makeText(ProductEditActivity.this, "商品描述输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_addTime.getText().toString().equals("")) {
						Toast.makeText(ProductEditActivity.this, "发布时间输入不能为空!", Toast.LENGTH_LONG).show();
						ET_addTime.setFocusable(true);
						ET_addTime.requestFocus();
						return;	
					}
					product.setAddTime(ET_addTime.getText().toString());
					/*调用业务逻辑层上传商品信息*/
	...
	private void initViewData() {
		...
		this.TV_productId.setText(productId+"");
		...
		for (int i = 0; i < sellerList.size(); i++) {
			if (product.getSellerObj().equals(sellerList.get(i).getSellUserName())) {
				this.spinner_sellerObj.setSelection(i);
				break;
			}
		}
		this.ET_addTime.setText(product.getAddTime());
	}
	@Override
