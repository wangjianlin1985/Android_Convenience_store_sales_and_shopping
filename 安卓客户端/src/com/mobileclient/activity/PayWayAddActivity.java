...
public class PayWayAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.payway_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加支付方式");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_payWayName.getText().toString().equals("")) {
						Toast.makeText(PayWayAddActivity.this, "支付方式名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传支付方式信息*/
					PayWayAddActivity.this.setTitle("正在上传支付方式信息，稍等...");
					String result = payWayService.AddPayWay(payWay);
