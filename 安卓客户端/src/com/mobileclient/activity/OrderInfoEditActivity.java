...
public class OrderInfoEditActivity extends Activity {
			...
			public void onClick(View v) {
				try {
					...
					if(ET_orderStateObj.getText().toString().equals("")) {
						Toast.makeText(OrderInfoEditActivity.this, "订单状态输入不能为空!", Toast.LENGTH_LONG).show();
	...
	private void initViewData() {
		...
		for (int i = 0; i < payWayList.size(); i++) {
			if (orderInfo.getPayWay() == payWayList.get(i).getPayWayId()) {
				this.spinner_payWay.setSelection(i);
				break;
			}
		}
		this.ET_orderStateObj.setText(orderInfo.getOrderStateObj());
		...
		for (int i = 0; i < sendWayList.size(); i++) {
			if (orderInfo.getSendWayObj() == sendWayList.get(i).getSendWayId()) {
				this.spinner_sendWayObj.setSelection(i);
				break;
			}
		}
		this.ET_orderMemo.setText(orderInfo.getOrderMemo());
		for (int i = 0; i < sellerList.size(); i++) {
			if (orderInfo.getSellerObj().equals(sellerList.get(i).getSellUserName())) {
