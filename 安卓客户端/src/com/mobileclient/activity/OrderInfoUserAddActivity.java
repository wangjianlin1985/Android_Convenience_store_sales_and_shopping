...
public class OrderInfoUserAddActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final Declare declare = (Declare)OrderInfoUserAddActivity.this.getApplication();
		UserInfo userInfo = userInfoService.GetUserInfo(declare.getUserName());
		this.ET_receiveName.setText(userInfo.getName()) ;
		...
		btnAdd.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					...
					orderInfo.setOrderNo(""); //服务器端生成订单号，涉及多个商家问题 
					 
					orderInfo.setTotalMoney(1.0f); //去服务器端计算 
					
					/*订单状态*/  
					orderInfo.setOrderStateObj("待付款");
					 
					/*设置下单用户*/
					orderInfo.setUserObj(declare.getUserName());
					
					/*下单时间*/ 
					orderInfo.setOrderTime(""); //下单时间服务器端生成 
					
					/*验证获取收货人*/ 
					if(ET_receiveName.getText().toString().equals("")) {
						Toast.makeText(OrderInfoUserAddActivity.this, "收货人输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_address.getText().toString().equals("")) {
						Toast.makeText(OrderInfoUserAddActivity.this, "收货人地址输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_orderMemo.getText().toString().equals("")) {
						Toast.makeText(OrderInfoUserAddActivity.this, "订单备注输入不能为空!", Toast.LENGTH_LONG).show();
					...
					orderInfo.setSellerObj("");
					
					/*调用业务逻辑层上传订单信息*/
					OrderInfoUserAddActivity.this.setTitle("正在上传订单信息，稍等...");
					String result = orderInfoService.AddOrderInfo(orderInfo);
					Toast.makeText(getApplicationContext(), result, 1).show(); 
					
					//下单完毕后去用户订单中心
					Intent intent = new Intent();
					intent.setClass(OrderInfoUserAddActivity.this, OrderInfoUserListActivity.class);
